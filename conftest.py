...
def pytest_addoption(parser):
    ...
    parser.addoption(
        '--host',
        default='dev',
        help='host options: "dev", "sandbox",'
             ' or your own host for local testing'
    )
...
def cluster(request):
    ...
    if cli_value in {'', 'dev'}:
        auth_provider = PlainTextAuthProvider(
            username=cacluster['dev']['username'],
            password=cacluster['dev']['password']
        )
        cluster = Cluster(
            cacluster['dev']['cluster'],
        ...
        auth_provider = PlainTextAuthProvider(
            username=cacluster['sandbox']['username'],
            password=cacluster['sandbox']['password']
        )
        cluster = Cluster(
            cacluster['sandbox']['cluster'],
            auth_provider=auth_provider,
            port=9042
        )
